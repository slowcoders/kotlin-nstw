...
open class JvmIrCodegenFactory(
    ...
    override fun invokeCodegen(input: CodegenFactory.CodegenInput) {
        ...
        if (state.configuration.languageVersionSettings.getFlag(JvmAnalysisFlags.outputBuiltinsMetadata)) {
    ...
    private fun serializeBuiltinsMetadata(module: IrModuleFragment, context: JvmBackendContext) {
        val allBuiltins = module.files.filter { it.isBuiltinWithoutBytecode || it.isBuiltinWithBytecode }
