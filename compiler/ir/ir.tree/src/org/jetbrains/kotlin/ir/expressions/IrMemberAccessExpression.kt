...
abstract class IrMemberAccessExpression<S : IrSymbol> : IrDeclarationReference() {
    ...
    internal fun initializeTargetShapeExplicitly(
        ...
        val existingContextArguments = arguments.subList(i, i + targetContextParameterCount).toMutableList()
        i += targetContextParameterCount
        val existingExtensionReceiver = if (targetHasExtensionReceiver) arguments[i++] else null
        val existingRegularArguments = arguments.subList(i, i + targetRegularParameterCount).toMutableList()
        ...
        existingContextArguments.setSize(contextParameterCount)
        arguments.addAll(existingContextArguments)
        if (hasExtensionReceiver) {
            arguments.add(existingExtensionReceiver)
        }
        existingRegularArguments.setSize(regularParameterCount)
        arguments.addAll(existingRegularArguments)
    }
