...
internal class VolatileFieldsLowering(val context: Context) : FileLoweringPass {
    ...
    override fun lower(irFile: IrFile) {
        irFile.transformChildrenVoid(object : IrBuildingTransformer(context) {
            ...
            private tailrec fun getConstPropertyReference(expression: IrExpression?, expectedReturn: IrReturnableBlockSymbol?) : IrBoundPropertyReference? {
                return when {
                    expression == null -> null
                    expectedReturn == null && expression is IrPropertyReference -> shouldNotBeCalled()
                    expectedReturn == null && expression is IrBoundPropertyReference -> expression
