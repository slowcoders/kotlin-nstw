...
internal fun ObjCExportCodeGeneratorBase.generateBlockToKotlinFunctionConverter(
    ...
    val objectBodyType = if (useSeparateHolder) {
        ObjectBodyType(
                llvm.structType(codegen.kObjHeader, llvm.pointerType),
...
private fun BlockType.toBlockInvokeLlvmType(llvm: CodegenLlvmHelpers): LlvmFunctionSignature =
        LlvmFunctionSignature(
                LlvmRetType(if (returnsVoid) llvm.voidType else llvm.pointerType, isObjectType = false),
                (0..numberOfParameters).map { LlvmParamType(llvm.pointerType) }
        )
...
internal class BlockGenerator(private val codegen: CodeGenerator) {
    ...
    private val disposeHelper = generateFunction(
        ...
        val refHolder = load(llvm.pointerType, refHolderInBlock(param(0)))
    ...
    internal fun ObjCExportCodeGeneratorBase.generateWrapKotlinObjectToRetainedBlock(
        ...
        return functionGenerator(
            ...
            val invoke = generateInvoke(blockType, invokeName, genBlockBody).llvm
            ...
            listOf(isa, flags, reserved, invoke, descriptor).forEachIndexed { index, value ->
            ...
            store(llvm.kNull, refHolderInBlock(blockOnStack))

            val copiedBlock = callFromBridge(retainBlock, listOf(blockOnStack))
