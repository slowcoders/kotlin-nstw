...
NO_INLINE void enqueHeapRefSlowPath(ObjHeader* obj) noexcept {
    AssertThreadState(ThreadState::kRunnable);

    if (obj != nullptr && obj->heap()) {
        ...
        gc::mark::ConcurrentMark::MarkTraits::tryEnqueue(markQueue, obj);
...
PERFORMANCE_INLINE void gc::barriers::nstw_heapRefUpdate(mm::DirectRefAccessor ref, ObjHeader* value) noexcept {
    ...
    if (__builtin_expect(phase == BarriersPhase::kMarkClosure, false)) {
        ObjHeader* erased = ref.exchange(value, std::memory_order_release);
        enqueHeapRefSlowPath(erased);
        enqueHeapRefSlowPath(value);
    } else {
        ref.store(value);
    }
}

PERFORMANCE_INLINE void gc::barriers::nstw_afterHeapRefUpdate(ObjHeader* erased, ObjHeader* value) noexcept {
    auto phase = currentPhase();
    BarriersLogDebug(phase, "Write %p (%p overwritten)", value, erased);
    if (__builtin_expect(phase == BarriersPhase::kMarkClosure, false)) {
        nstw_afterHeapRefUpdate(erased, value);
    }
}
...
PERFORMANCE_INLINE void gc::barriers::afterSpecialRefReleaseToZero(mm::DirectRefAccessor ref) noexcept {
    ...
    if (__builtin_expect(phase == BarriersPhase::kMarkClosure, false)) {
        enqueHeapRefSlowPath(ref.loadAtomic(std::memory_order_relaxed));
    }
}
