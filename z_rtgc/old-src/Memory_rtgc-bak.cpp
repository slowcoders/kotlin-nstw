  ...
  if (RTGC && doFree) {
    ...
    while (true) {
      ...
      if (obj == nullptr) {
        ...
        rtgcRefAssert(deassigned, !isAggregatingFrozenContainer(deassigned));
    ...
    processFinalizerQueue(memState);
      ...
      rtgcAssert(container->objectCount() == 1);
      ...
      while (true) {//garbageNodeId == 0) {
        ...
        traverseReferredObjects((ObjHeader*)(owner+1), [garbageNodeId, toRelease, &isOwnerPushed, owner](ObjHeader* old) {
          ...
          rtgcAssert(deassigned == NULL || deassigned->objectCount() == 1);
...
void freezeSubgraph(ObjHeader* root) {
  ...
  for (auto* e: newlyFrozen) {
    ...
    rtgcAssert(container->isFreezing());
